          ...
          {aiPieData.length === 0 ? (
            ...
            <>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip
                      ...
                      contentStyle={{ background: 'var(--bg-glass)', border: '1px solid var(--border-glass)', borderRadius: 12 }}
                    />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              <div className="flex justify-center gap-4 mb-3">
                {aiPieData.map((d, i) => (
                  <div key={d.name} className="flex items-center gap-1.5 text-[0.7rem]">
                    <div className="w-2 h-2 rounded-full" style={{ background: i === 0 ? 'var(--accent-emerald)' : 'var(--accent-rose)' }} />
                    <span style={{ color: 'var(--text-dim)' }}>{d.name}: <strong style={{ color: 'var(--text-main)' }}>{d.value}</strong></span>
                  </div>
                ))}
              </div>
