...
import { Plus, Search, Trash2, Edit3, X, Check, Layers, MapPin, Calendar, DollarSign, RefreshCw, Package, Building2, ChevronLeft, ChevronRight, AlertTriangle } from 'lucide-react';

const PAGE_SIZE = 10;

const BLANK = { name: '', type: '', location: '', ownerDepartment: '', purchaseDate: '', purchasePrice: '', userId: '' };
...
export default function Resources() {
  ...
  const handleReportDamage = (r) => {
    navigate(`/damage-reports?resourceId=${r.id}`);
  };

  // Base pool of resources:
  // Natively scoped by the backend API based on user role.
    ...
    <div className="animate-in">
        ...
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: 'var(--text-muted)' }}>
            {isAdmin
              ...
              : isStaff
                ? `Showing assets assigned to you${userDept ? ` (${userDept})` : ''}.`
      ...
      <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', marginBottom: '1.5rem' }}>
        <div className="glass-card" style={{ padding: '1.25rem' }}>
          <p className="text-[0.65rem] font-bold uppercase tracking-wider mb-1" style={{ color: 'var(--accent-blue)' }}>
            {isStaff ? 'My Assigned Assets' : isAdmin ? 'Total University Assets' : 'Departmental Assets'}
          </p>
          <h2 className="text-3xl font-black">{baseResources.length}</h2>
        </div>
      </div>

      {/* Table */}
      <div className="glass-card overflow-hidden" style={{ padding: 0, marginTop: '1.5rem' }}>
        <div className="table-search-bar" style={{ display: 'block' }}>
          <div className="search-wrap" style={{ position: 'relative', width: '100%' }}>
            ...
            <input className="input-glass" style={{ paddingLeft: '2.25rem', width: '100%' }} placeholder="Search by name, type or location..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
          ...
          <table className="w-full" style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-glass)' }}>
                {(() => {
                  const isReportAllowed = ['STAFF', 'DEPARTMENT_HEAD', 'ACADEMIC_DEAN'].includes(user?.role);
                  const showActions = canManage || isReportAllowed;
                  return ['Asset Identity', 'Type', 'Location', 'Ownership', 'Assigned To', 'Purchased', 'Value', showActions ? 'Actions' : ''];
                })().map(h => (
                  h ? <th key={h} style={{ textAlign: 'left', padding: '0.7rem 1rem', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-dim)', whiteSpace: 'nowrap' }}>{h}</th> : <th key="empty" />
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={r.id} className="table-row-hover" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                    ...
                    <td className="px-4 py-3 text-sm" style={{color:'var(--text-dim)'}}>
                      {r.user ? (
                        <span className="font-semibold" style={{color:'var(--text-main)'}}>{r.user.name}</span>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex gap-1.5 justify-end">
                        {isReportAllowed && (
                          <button onClick={() => handleReportDamage(r)}
                            className="px-2.5 py-1.5 rounded-lg cursor-pointer text-xs font-bold flex items-center gap-1 shrink-0"
          ...
          <div className="glass-card modal-card animate-in" style={{ maxWidth: 520 }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2">
                <label className="text-[0.7rem] font-bold uppercase block mb-1" style={{ color: 'var(--text-dim)' }}>Assigned To (Optional)</label>
                <select className="input-glass" value={form.userId} onChange={e => setForm(f => ({ ...f, userId: e.target.value }))}>
                  <option value="">— Unassigned / Department-wide —</option>
                  {users.map(u => (
